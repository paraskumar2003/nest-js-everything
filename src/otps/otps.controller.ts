...
export class OtpsController {
  ...
  async create(@Body() createOtpDto: CreateOtpDto) {
    ...
    this.logger.info('CREATE_OTP_REQUEST', journeyId, { dto: createOtpDto });
  ...
  @Get()
  async findAll(@Query('mobile') mobile?: string, @Query('verified') verified?: string) {
    const journeyId = uuidv4();
    ...
    this.logger.info('GET_OTPS_RESPONSE', journeyId, { count: result.length });
    return result;
  }
